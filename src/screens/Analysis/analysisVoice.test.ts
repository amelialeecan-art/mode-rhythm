...
describe('factorPhrase — 강도별 편한 말투', () => {
  it('강함(지연): 직설 인과 + 반복 표현', () => {
    ...
    expect(r.text).toBe('대인 갈등 때문에 그 뒤 며칠간 잠이 망가지는 패턴이 반복됐어.')
  })
  ...
  it('중간(지연): 인과 단정 없이 경향', () => {
    ...
    expect(r.text).toBe('쇼츠·릴스 과다 뒤 몸이 힘든 날이 많았어.')
...
describe('factorPhrase — 같은 날 관계는 잘못된 선후로 바꾸지 않음', () => {
  ...
  it('같은 날은 evidence가 높아도 인과 표현 없음', () => {
    ...
    expect(r.text).toContain('같은 날 같이 터졌어')
  })
})
...
describe('episodeTrigger — 유쾌 라벨(카드당 1회), 결정론적', () => {
  it('선행 2개면 조합 트리거', () => {
    expect(episodeTrigger({ precursors: ['마감·압박', '수면 부족'], afters: [] })).toBe(
      '이번 정병 트리거는 마감·압박과 수면 부족 조합이었어.',
    )
  })
  it('선행 1개면 그 쪽', () => {
    expect(episodeTrigger({ precursors: ['대인 갈등'], afters: [] })).toBe('이번 정병 트리거는 대인 갈등 쪽이 컸어.')
  })
