...
describe('rhythmCompareSentence — 최근 일주일 자연어', () => {
  it('부하 metric 증가 → 많았어', () => {
    expect(rhythmCompareSentence('sleep', stat(12))).toBe('최근 일주일은 평소보다 수면 문제가 많았어.')
    expect(rhythmCompareSentence('emotional', stat(20))).toBe('최근 일주일은 평소보다 감정 흔들림이 많았어.')
  })
  it('부하 metric 감소 → 적었어', () => {
    expect(rhythmCompareSentence('emotional', stat(-12))).toBe('최근 일주일은 평소보다 감정 흔들림이 적었어.')
  })
  it('차이 작으면 → 큰 차이 없음', () => {
    expect(rhythmCompareSentence('body', stat(3))).toBe('최근 일주일은 평소와 큰 차이가 없었어.')
  })
  it('회복은 늘었/줄었 표현', () => {
    expect(rhythmCompareSentence('recovery', stat(12))).toBe('최근 일주일은 평소보다 회복 행동이 늘었어.')
    expect(rhythmCompareSentence('recovery', stat(-12))).toBe('최근 일주일은 평소보다 회복 행동이 줄었어.')
  })
  it('표본 부족이면 안내 문장', () => {
    expect(rhythmCompareSentence('sleep', stat(30, false))).toContain('기록이 조금 적어')
  })
...
describe('cycleCompareSentence — 주기 비교', () => {
  it('이전 주기가 같은 시기 안정적이면 억울 표현', () => {
    ...
    expect(cycleCompareSentence('emotional', c)).toBe(
      '생리 탓만 하기엔 억울해. 이전 주기에는 같은 시기에도 감정이 비교적 안정적이었어.',
    )
  })
  ...
  it('이전보다 일찍 흔들리기 시작하면 며칠 빨랐다고', () => {
    ...
    expect(s).toContain('이번 주기는 생리 6일 전부터 수면이 흔들렸어.')
    expect(s).toContain('이전 주기보다 4일쯤 빨랐어.')
  })

  it('비슷하면 거의 비슷했다고', () => {
    const c = curve(45, flat(50), flat(50))
    expect(cycleCompareSentence('appetite', c)).toBe('식욕 변화는 이전 주기들과 거의 비슷했어.')
  })

  it('회복은 늘었/줄었', () => {
    const up = curve(40, flat(70), flat(45))
    expect(cycleCompareSentence('recovery', up)).toBe('이번 주기는 이전보다 회복 행동이 많았어.')
  })
...
describe('personalRhythmSentence — 표시 정리 (9H)', () => {
  ...
  it('내부 depleting 시작이어도 표시는 안정 중심으로 자연스럽게', () => {
    ...
    expect(lines[0]).toBe('최근 기록에서는 안정된 기간을 사이에 두고 소모와 회복 흐름이 반복됐어.')
    expect(lines[0]).not.toMatch(/^최근 기록에서는 소모/) // 소모를 시작점처럼 보이지 않음
  })

  it('min===max면 52~52일이 아니라 약 52일', () => {
    const lines = personalRhythmSentence(base)
    expect(lines.some((l) => l.includes('한 흐름은 약 52일 이어졌어.'))).toBe(true)
    expect(lines.join(' ')).not.toContain('52~52')
  })

  it('currentMatch 문장은 내부 현재 상태(소모)를 그대로 유지', () => {
    const lines = personalRhythmSentence(base)
    expect(lines.some((l) => l.includes('지금은 반복 흐름 중 소모 구간에 있어'))).toBe(true)
    expect(lines.some((l) => l.includes('수면과 몸 상태가 먼저 내려갔어'))).toBe(true)
  })

  it('서로 다른 상태만이면 화살표(안정부터 회전)', () => {
    const r: PersonalRhythm = { ...base, sequence: ['depleting', 'recovering', 'stable'], currentMatch: null }
    expect(personalRhythmSentence(r)[0]).toBe('최근 기록에서는 안정 → 소모 → 회복 흐름이 반복됐어.')
  })
...
describe('monthlyComparisonView (step5)', () => {
  ...
  it('(17) 진행 중인 달 문장에 "지난달 같은 기간"을 쓴다', () => {
    ...
    expect(v.lines[0]).toBe('몸이 쉽게 지친 날이 줄었어')
  })
  ...
  it('완료된 달은 "N월은 M월보다" 형식', () => {
    ...
    expect(v.lines[0]).toBe('평소 하던 일이 버거운 날이 줄었어')
  })
  ...
  it('개선과 악화가 함께 있으면 모두 문장으로 표현한다', () => {
    ...
    expect(v.lines).toContain('평소 하던 일이 버거운 날이 줄었어')
    expect(v.lines).toContain('단것 당김이 강한 날이 늘었어')
  })
  ...
  it('생활 맥락은 좋고 나쁨 없이 사실로 표현', () => {
    ...
    expect(v.lines[0]).toBe('출근일 비중이 늘었어')
  })
