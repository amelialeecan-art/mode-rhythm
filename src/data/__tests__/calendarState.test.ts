...
describe('Calendar 상태 라벨 (update1)', () => {
  // (1) 새 직접 입력 필드가 있으면 "저장된 상태 기록이 없어"가 나오지 않는다
  it('새 직접 입력만 있어도 상태 라벨이 표시된다', async () => {
    ...
    expect(detail.stateLabels).toContain('집중이 잘 됐어')
  })
  ...
  it('몸 에너지·머릿속·집중·사회 여유·생활기능이 사람말로 표시된다', async () => {
    ...
    expect(stateLabels).toContain('몸이 쉽게 지쳤어')
    expect(stateLabels).toContain('머리가 복잡했어')
    expect(stateLabels).toContain('집중하기 어려웠어')
    expect(stateLabels).toContain('사람을 대하기 버거웠어')
    expect(stateLabels).toContain('평소 하던 일이 버거웠어')
    // 개발자식 필드명은 노출하지 않는다
  ...
  it('입력하지 않은 항목을 정상으로 채우지 않는다', async () => {
    ...
    const { stateLabels } = await getCalendarDayDetail(D)
    expect(stateLabels).toEqual(['몸이 쉽게 지쳤어'])
  })
